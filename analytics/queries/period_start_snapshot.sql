-- Start-of-period snapshot (PHASE_0 §2): cohort = ≥1 audit row during [start,end] + product ({{DATE_RANGE_FILTER}}).
-- Latest mapped stage using only rows with timestamp::DATE strictly BEFORE period start calendar day (naive).
-- step_order 0 = cohort member with no qualifying prior row (often overlaps “arrivals”; see period_arrivals_losses).
-- Unit: one application per bucket (§5 funnel / in-filter spirit).
...
prior AS (
    ...
    WHERE {{PRODUCT_TYPE_FILTER_A}}
      AND a.timestamp::DATE < {{PERIOD_START_DATE}}
...
labeled AS (
    SELECT
        c.application_id,
        COALESCE(p.step_order, 0)::INTEGER AS step_order,
        p.last_event_at AS last_event_at
    FROM cohort AS c
    LEFT JOIN prior AS p ON p.application_id = c.application_id
),
agg AS (
    SELECT
        step_order,
        ...
        AVG(
            CASE
                WHEN last_event_at IS NOT NULL
                THEN date_diff('day', last_event_at, current_timestamp)::DOUBLE
            END
