-- SLA compliance (v0).
--
-- SLAs (eligible rows are scoped by the START event falling in the selected date window):
--   1) CR review:        CUSTOMER_RELATION_REVIEW_STARTED -> CUSTOMER_RELATION_REVIEW_COMPLETED  (<= 24h)
--   2) Compliance:       COMPLIANCE_REVIEW_STARTED        -> COMPLIANCE_REVIEW_COMPLETED        (<= 48h)
--   3) RFI response:     INTERACTION_STARTED              -> INTERACTION_SUBMITTED              (<= 72h)
--   4) Offer issuance:   COMPLIANCE_REVIEW_COMPLETED      -> OFFER_SENT                          (<= 5d)
--
-- For open cases (missing end event), status is evaluated as-of now.
...
SELECT
    ...
    SUM(CASE WHEN elapsed_hours <= threshold_hours THEN 1 ELSE 0 END)::BIGINT AS n_within,
    SUM(CASE WHEN is_open AND elapsed_hours > (0.8 * threshold_hours) AND elapsed_hours < threshold_hours THEN 1 ELSE 0 END)::BIGINT AS n_at_risk,
    SUM(CASE WHEN elapsed_hours > threshold_hours THEN 1 ELSE 0 END)::BIGINT AS n_breached,
...
ORDER BY
    CASE sla_name
        ...
        WHEN 'RFI response' THEN 3
        WHEN 'Offer issuance' THEN 4
        ELSE 99
    END;
